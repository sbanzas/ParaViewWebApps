(function (GLOBAL, $) {
   ...
   // ------------------------------------------------------------------------
    ...
    function initializeVisualizer(session_, viewportSelector, pipelineSelector, proxyEditorSelector, fileSelector, sourceSelector, filterSelector, dataInfoSelector,filePath) {
        session = session_;
        ...
        $('.need-input-source').hide();
        proxyEditor.empty();
        session.call("pv.proxy.manager.create.reader", [filePath]).then(invalidatePipeline, invalidatePipeline);
        activePipelineInspector();
    };
